...
async def test_login_wrong_password_returns_401(anon_client):
    ...
    await anon_client.post("/auth/register", json=payload)
    res = await anon_client.post(
        "/auth/login", json={"email": payload["email"], "password": "wrongpassword"}
    )
...
async def test_login_errors_have_same_message_for_wrong_password_and_missing_user(
    anon_client,
):
...
async def test_access_token_authorises_protected_routes(anon_client):
    ...
    res = await anon_client.get(
        "/quizzes", headers={"Authorization": f"Bearer {access}"}
    )
    assert res.status_code == 200
